...
import type { BuildSystemId } from '../buildsystem/detect.js';
 ...
 * before anything is. This module only reads, and it reads only manifest *names* —
 * it opens no build file, runs nothing, and never scans a tree recursively.
 *
 * The declaration it proposes is derived from what the chosen profile can actually
 * supply, so a generated file validates and resolves on the first try. Writing out a
 * template that then fails validation would make the first experience of the harness
 * a schema error.
 */
...
function declaration(name: string, profileId: string, gates: Set<string>, context: string[]): string {
...
#
# Only the gates this profile supplies a command for are enabled. Enable another
# gate by declaring its command here as well; a gate with no command is refused by
  ...
  const proposed: { path: string; contents: string; note?: string }[] = [
    {
      ...
      contents: declaration(name, profile.id, gates, contextKeys),
      note: `${gates.size} gate(s) enabled, being those ${profile.id} supplies a command for`,
    },
    ...contextKeys.map(key => ({
